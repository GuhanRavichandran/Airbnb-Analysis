...
st.caption(":rainbow[***Project observation help us to know occupancy of rooms and customer rating and experience on stay***]:airplane_arriving:")
st.write("")
import base64
...
background_image_path = r'C:\Users\Hp\OneDrive\Desktop\airbnb.py\image-airbnb.jpg'
...
def datafr():
    try:
        df = pd.read_csv(r"C:\Users\Hp\OneDrive\Desktop\airbnb.py\Airbnb.csv", encoding='ISO-8859-1')
        return df
    except UnicodeDecodeError:
        print("Encoding error: please try a different encoding.")
        raise

df= datafr()

select= st.selectbox(":red[Main Menu]",["Airbnb_info", "Explorated_Analysis", "Process_Executed"])

if select == "Airbnb_info":



    st.header(":red[About Airbnb]")
    st.write("")
    st.write(''':green[***Airbnb is an online marketplace that connects people who want to rent out
    ...
    st.write("")
    st.image(r"C:\Users\Hp\OneDrive\Desktop\airbnb.py\airbnb.jpeg", use_column_width=True)
    st.write(''':green[***Airbnb Inc (Airbnb) operates an online platform for hospitality services.
    ...
    st.header(":red[Background of Airbnb]")
    st.write("")
    st.write(''':green[***Airbnb was born in 2007 when two Hosts welcomed three guests to their
    ...
    st.image(r"C:\Users\Hp\OneDrive\Desktop\airbnb.py\airbnb2.jpeg", use_column_width=True)


if select == "Explorated_Analysis":
    tab1, tab2, tab3, tab4, tab5= st.tabs(["***PRICE ANALYSIS***","***AVAILABILITY ANALYSIS***","***LOCATION BASED***", "***GEOSPATIAL VISUALIZATION***", "***TOP CHARTS***"])
    with tab1:
        st.title(":orange[**PRICE DIFFERENCE**]:currency_exchange:")
        col1,col2= st.columns(2)

        with col1:
            
            
            country= st.selectbox(":red[Select the Country]",df["country"].unique())

            df1= df[df["country"] == country]
            df1.reset_index(drop= True, inplace= True)

            room_ty= st.selectbox(":red[Select the Room Type]",df1["room_type"].unique())
        ...
        with col2:
            ...
            proper_ty= st.selectbox(":red[Select the Property_type]",df2["property_type"].unique())
        ...
        with col1:
            ...
            hostresponsetime= st.selectbox(":red[Select the host_response_time]",df4["host_response_time"].unique())
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv(r"C:\Users\Hp\OneDrive\Desktop\airbnb.py\Airbnb.csv")
            return df_a

        df_a= datafr()

        st.title(":orange[**AVAILABILITY ANALYSIS**]:hotel:")
        col1,col2= st.columns(2)

        with col1:
            
            
            country_a= st.selectbox(":red[Select the Country_a]",df_a["country"].unique())

            df1_a= df[df["country"] == country_a]
            df1_a.reset_index(drop= True, inplace= True)

            property_ty_a= st.selectbox(":red[Select the Property Type]",df1_a["property_type"].unique())
        ...
        roomtype_a= st.selectbox(":red[Select the Room Type_a]", df2_a["room_type"].unique())
    ...
    with tab3:
        ...
        st.title(":orange[LOCATION ANALYSIS]:man-walking:")
        st.write("")

        def datafr():
            df= pd.read_csv("C:/Users/Hp/OneDrive/Desktop/airbnb.py/Airbnb.csv")
            return df

        df_l= datafr()

        country_l= st.selectbox(":red[Select the Country_l]",df_l["country"].unique())

        df1_l= df_l[df_l["country"] == country_l]
        df1_l.reset_index(drop= True, inplace= True)

        proper_ty_l= st.selectbox(":red[Select the Property_type_l]",df1_l["property_type"].unique())
        ...
        val_sel= st.radio(":red[Select the Price Range]",[str(df2_l['price'].min())+' '+str('to')+' '+str(differ_max_min*0.30 + df2_l['price'].min())+' '+str("(30% of the Value)"),
        ...
        room_ty_l= st.selectbox(":red[Select the Room_Type_l]", df_val_sel["room_type"].unique())

        df_val_sel_rt= df_val_sel[df_val_sel["room_type"] == room_ty_l]

        df_long = df_val_sel_rt.melt(id_vars=["market"], value_vars=["street", "host_location", "host_neighbourhood"],
                                    var_name="Category", value_name="Value")

        # Plot
        fig_2 = px.bar(df_long, x="Value", y="market", color="Category",
                    title="MARKET", hover_data=["Value", "market"],
                    barmode='group', orientation='h', color_discrete_sequence=px.colors.sequential.Rainbow_r, width=1000)
        st.plotly_chart(fig_2)

        df_val_sel_rt["government_area"] = df_val_sel_rt["government_area"].astype(str)

        # Plot for each metric
        for metric in ["host_is_superhost", "host_neighbourhood", "cancellation_policy"]:
            fig_3 = px.bar(df_val_sel_rt, x="government_area", y=metric,
                        title=f"GOVERNMENT_AREA - {metric}",
    ...
    with tab4:
        ...
        st.title(":blue[GEOSPATIAL VIEW]:earth_africa:")
        st.write("")
    ...
    with tab5:
        st.title(":red[TOP CHARTS]:chart:")
        country_t= st.selectbox(":red[Select the Country_t]",df["country"].unique())

        df1_t= df[df["country"] == country_t]

        property_ty_t= st.selectbox(":red[Select the Property_type_t]",df1_t["property_type"].unique())
        ...
        room_type_t= st.selectbox(":red[Select the Room_Type_t]",df2_t_sorted["room_type"].unique())
...
if select == "Process_Executed":

    st.header(":green[ABOUT THIS PROJECT]")

    st.subheader(":orange[1. Data Collection:]")

    st.write(''':green[***Gather data from Airbnb's public API or other available sources.
    ...
    st.write(''':green[***Clean and preprocess the data to handle missing values, outliers, and ensure data quality.
        Convert data types, handle duplicates, and standardize formats.***]''')
    
    st.subheader(":orange[3. Exploratory Data Analysis (EDA):]")

    st.write(''':green[***Conduct exploratory data analysis to understand the distribution and patterns in the data.
        Explore relationships between variables and identify potential insights.***]''')
    
    st.subheader(":orange[4. Visualization:]")

    st.write(''':green[***Create visualizations to represent key metrics and trends.
    ...
    st.write(''':green[***Utilize geospatial analysis to understand the geographical distribution of listings.
